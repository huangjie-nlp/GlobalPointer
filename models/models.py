...
class GlobalPointer(nn.Module):
    def __init__(self, config):
        ...
        self.bert_dim = 768
        self.linear = nn.Linear(self.bert_dim, self.config.num_type * self.config.hidden_size * 2)
    ...
    def forward(self, data):
        ...
        last_hidden_state = self.bert(input_ids, attention_mask=attention_mask)[0]
        ...
        # [batch_size, num_type, seq_len, seq_len]
        pad_mask = attention_mask.unsqueeze(1).unsqueeze(1).expand(batch_size, self.config.num_type, seq_len, seq_len)
